...
def dashboard():
    ...
    stats = {
        ...
        "pending": conn.execute(
            "SELECT COUNT(*) c FROM transactions WHERE status='قيد الإجراء'"
        ).fetchone()["c"],
        "balance": conn.execute(
            "SELECT COALESCE(SUM(total-paid),0) x FROM transactions"
        ).fetchone()["x"],
    }
...
def client_profile(client_id):
    ...
    transactions = conn.execute("""SELECT * FROM transactions
        WHERE client_id=? ORDER BY id DESC""", (client_id,)).fetchall()

    bookings = conn.execute("""SELECT * FROM bus_bookings
        WHERE client_id=? ORDER BY id DESC""", (client_id,)).fetchall()

    financial = conn.execute("""SELECT
        COALESCE(SUM(total),0) AS total,
        COALESCE(SUM(paid),0) AS paid,
        COALESCE(SUM(expenses),0) AS expenses,
        COALESCE(SUM(total-paid),0) AS remaining,
        COALESCE(SUM(total-paid-expenses),0) AS profit
        FROM transactions WHERE client_id=?""", (client_id,)).fetchone()

    bus_financial = conn.execute("""SELECT
        COALESCE(SUM(total),0) AS total,
        COALESCE(SUM(paid),0) AS paid,
        COALESCE(SUM(total-paid),0) AS remaining
        FROM bus_bookings WHERE client_id=?""", (client_id,)).fetchone()

    conn.close()

    totals = {
        "total": (financial["total"] or 0) + (bus_financial["total"] or 0),
        "paid": (financial["paid"] or 0) + (bus_financial["paid"] or 0),
        "remaining": (financial["remaining"] or 0) + (bus_financial["remaining"] or 0),
...
def delete_client(client_id):
    conn = db()
    count = conn.execute(
        "SELECT COUNT(*) c FROM transactions WHERE client_id=?", (client_id,)
    ).fetchone()["c"]
    count += conn.execute(
        "SELECT COUNT(*) c FROM bus_bookings WHERE client_id=?", (client_id,)
    ).fetchone()["c"]
...
def add_transaction():
    ...
    clients = conn.execute(
        "SELECT id,name,passport FROM clients ORDER BY name"
    ).fetchall()

    if request.method == "POST":
        conn.execute("""INSERT INTO transactions
        (client_id,service,status,application_no,issue_date,entry_date,exit_date,
         total,paid,expenses,notes,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", (
            request.form["client_id"], request.form["service"],
            request.form["status"], request.form.get("application_no"),
            request.form.get("issue_date"), request.form.get("entry_date"),
            request.form.get("exit_date"),
            float(request.form.get("total") or 0),
            float(request.form.get("paid") or 0),
            float(request.form.get("expenses") or 0),
            request.form.get("notes"),
            datetime.now().isoformat(timespec="seconds")
        ))
        conn.commit()
...
def edit_transaction(transaction_id):
    ...
    if request.method == "POST":
        conn.execute("""UPDATE transactions SET client_id=?, service=?, status=?,
            ...
            float(request.form.get("paid") or 0),
            float(request.form.get("expenses") or 0),
            request.form.get("notes"), transaction_id
        ))
        conn.commit()
        conn.close()
...
def add_bus_booking():
    ...
    if request.method == "POST":
        conn.execute("""INSERT INTO bus_bookings
            (client_id,company,booking_no,from_city,to_city,travel_date,seats,
             total,paid,status,notes,created_at)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""", (
            request.form["client_id"], request.form["company"],
            request.form.get("booking_no"), request.form.get("from_city"),
            request.form.get("to_city"), request.form.get("travel_date"),
            int(request.form.get("seats") or 1),
            float(request.form.get("total") or 0),
            float(request.form.get("paid") or 0),
...
def edit_bus_booking(booking_id):
    conn = db()
    b = conn.execute(
        "SELECT * FROM bus_bookings WHERE id=?", (booking_id,)
    ).fetchone()
    clients = conn.execute(
        "SELECT id,name,passport FROM clients ORDER BY name"
    ).fetchall()
...
def delete_bus_booking(booking_id):
    conn = db()
    conn.execute("DELETE FROM bus_bookings WHERE id=?", (booking_id,))
    conn.commit()
    conn.close()
    flash("تم حذف الحجز")
    return redirect(url_for("bus_bookings"))
