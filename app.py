...
def init_db():
    ...
    # Render قد يبدأ بقاعدة SQLite جديدة بعد إعادة النشر؛ أنشئ حسابًا أوليًا
    # مرة واحدة فقط عند عدم وجود أي مستخدم. يمكن تخصيصه من متغيرات Render.
    ...
    if users_count == 0:
        ...
        conn.execute(
            ...
            (admin_username, generate_password_hash(admin_password, method="scrypt")),
        )
    # استرجاع طارئ لمرة واحدة عند تعذر الدخول إلى قاعدة موجودة.
    # يجب حذف RESET_ADMIN_PASSWORD من Render بعد نجاح الدخول.
        ...
        if existing:
            conn.execute("UPDATE users SET password_hash=? WHERE id=?", (hashed, existing["id"]))
        else:
            conn.execute("INSERT INTO users (username, password_hash) VALUES (?,?)", (admin_username, hashed))
    conn.commit()
    conn.close()
