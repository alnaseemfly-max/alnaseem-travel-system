...
def load_user(user_id):
    ...
    conn.close()
    return User(u["id"], u["username"], u["password_hash"]) if u else None
...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS transactions (
        ...
        application_no TEXT, issue_date TEXT, entry_date TEXT, exit_date TEXT,
        total REAL DEFAULT 0, paid REAL DEFAULT 0, expenses REAL DEFAULT 0,
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS bus_bookings (
        ...
        booking_no TEXT, from_city TEXT, to_city TEXT, travel_date TEXT,
        seats INTEGER DEFAULT 1, total REAL DEFAULT 0, paid REAL DEFAULT 0,
        status TEXT NOT NULL DEFAULT 'قيد الإجراء', notes TEXT, created_at TEXT NOT NULL,
        FOREIGN KEY(client_id) REFERENCES clients(id)
    )""")

    # ينشئ المدير فقط إذا لم يوجد أي مستخدم. لا يغيّر كلمة مرور حساب موجود.
    if conn.execute("SELECT COUNT(*) c FROM users").fetchone()["c"] == 0:
        conn.execute(
            "INSERT INTO users (username, password_hash) VALUES (?, ?)",
            ("admin", generate_password_hash("admin123", method="scrypt"))
        )
...
def change_password():
    if request.method == "POST":
        ...
        conn.execute("UPDATE users SET password_hash=? WHERE id=?",
                     (generate_password_hash(new, method="scrypt"), current_user.id))
...
def clients():
    ...
    if q:
        rows = conn.execute("""SELECT * FROM clients
            WHERE name LIKE ? OR passport LIKE ? OR phone LIKE ? ORDER BY id DESC""",
...
def client_profile(client_id):
    ...
    transactions = conn.execute("SELECT * FROM transactions WHERE client_id=? ORDER BY id DESC",
                                (client_id,)).fetchall()
    bookings = conn.execute("SELECT * FROM bus_bookings WHERE client_id=? ORDER BY id DESC",
                            (client_id,)).fetchall()
    f = conn.execute("""SELECT COALESCE(SUM(total),0) total, COALESCE(SUM(paid),0) paid,
        COALESCE(SUM(expenses),0) expenses,
        COALESCE(SUM(total-paid),0) remaining,
        COALESCE(SUM(total-paid-expenses),0) profit
        FROM transactions WHERE client_id=?""", (client_id,)).fetchone()
    b = conn.execute("""SELECT COALESCE(SUM(total),0) total, COALESCE(SUM(paid),0) paid,
        COALESCE(SUM(total-paid),0) remaining FROM bus_bookings WHERE client_id=?""",
                     (client_id,)).fetchone()
    conn.close()
    totals = {
        "total": (f["total"] or 0)+(b["total"] or 0),
        "paid": (f["paid"] or 0)+(b["paid"] or 0),
        "remaining": (f["remaining"] or 0)+(b["remaining"] or 0),
        "expenses": f["expenses"] or 0, "profit": f["profit"] or 0
    }
    return render_template("client_profile.html", client=client,
                           transactions=transactions, bookings=bookings, totals=totals)
...
def edit_client(client_id):
    ...
    if request.method=="POST":
        name=request.form.get("name","").strip()
        if not name:
            conn.close(); flash("الاسم مطلوب"); return redirect(url_for("edit_client",client_id=client_id))
        conn.execute("""UPDATE clients SET name=?, passport=?, phone=?, notes=? WHERE id=?""",
                     (name,request.form.get("passport"),request.form.get("phone"),
...
def delete_client(client_id):
    ...
    if count: flash("لا يمكن حذف العميل لأنه مرتبط بمعاملات أو حجوزات")
    else:
...
def transactions():
    ...
    conn.close(); return render_template("transactions.html", transactions=rows)

@app.route("/transactions/add", methods=["GET","POST"])
@login_required
def add_transaction():
    conn=db(); clients=conn.execute("SELECT id,name,passport FROM clients ORDER BY name").fetchall()
    if request.method=="POST":
        ...
        conn.commit(); conn.close(); flash("تمت إضافة المعاملة")
        return redirect(url_for("transactions"))
    conn.close(); return render_template("transaction_form.html",clients=clients,transaction=None)

@app.route("/transactions/<int:transaction_id>/edit", methods=["GET","POST"])
@login_required
def edit_transaction(transaction_id):
    conn=db(); t=conn.execute("SELECT * FROM transactions WHERE id=?", (transaction_id,)).fetchone()
    ...
    if request.method=="POST":
        conn.execute("""UPDATE transactions SET client_id=?,service=?,status=?,application_no=?,
            issue_date=?,entry_date=?,exit_date=?,total=?,paid=?,expenses=?,notes=? WHERE id=?""", (
            request.form["client_id"],request.form["service"],request.form["status"],
            request.form.get("application_no"),request.form.get("issue_date"),
            request.form.get("entry_date"),request.form.get("exit_date"),
            float(request.form.get("total") or 0),float(request.form.get("paid") or 0),
            float(request.form.get("expenses") or 0),request.form.get("notes"),transaction_id))
        conn.commit(); conn.close(); flash("تم تعديل المعاملة")
        return redirect(url_for("transactions"))
    conn.close(); return render_template("transaction_form.html",clients=clients,transaction=t)
...
def bus_bookings():
    conn=db()
    rows=conn.execute("""SELECT b.*,c.name client_name,c.passport FROM bus_bookings b
                         JOIN clients c ON c.id=b.client_id ORDER BY b.id DESC""").fetchall()
    conn.close(); return render_template("bus_bookings.html",bookings=rows)
...
def add_bus_booking():
    ...
    if request.method=="POST":
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""", (
            ...
            request.form.get("from_city"),request.form.get("to_city"),request.form.get("travel_date"),
            int(request.form.get("seats") or 1),float(request.form.get("total") or 0),
            float(request.form.get("paid") or 0),request.form["status"],request.form.get("notes"),
            datetime.now().isoformat(timespec="seconds")))
        conn.commit(); conn.close(); flash("تمت إضافة حجز الباص")
        return redirect(url_for("bus_bookings"))
    conn.close(); return render_template("bus_booking_form.html",clients=clients,booking=None)

@app.route("/bus-bookings/<int:booking_id>/edit", methods=["GET","POST"])
@login_required
def edit_bus_booking(booking_id):
    conn=db(); b=conn.execute("SELECT * FROM bus_bookings WHERE id=?", (booking_id,)).fetchone()
    clients=conn.execute("SELECT id,name,passport FROM clients ORDER BY name").fetchall()
    if not b:
        conn.close(); flash("الحجز غير موجود"); return redirect(url_for("bus_bookings"))
    if request.method=="POST":
        conn.execute("""UPDATE bus_bookings SET client_id=?,company=?,booking_no=?,from_city=?,
            to_city=?,travel_date=?,seats=?,total=?,paid=?,status=?,notes=? WHERE id=?""", (
            request.form["client_id"],request.form["company"],request.form.get("booking_no"),
            request.form.get("from_city"),request.form.get("to_city"),request.form.get("travel_date"),
            int(request.form.get("seats") or 1),float(request.form.get("total") or 0),
            float(request.form.get("paid") or 0),request.form["status"],request.form.get("notes"),booking_id))
        conn.commit(); conn.close(); flash("تم تعديل حجز الباص")
        return redirect(url_for("bus_bookings"))
    conn.close(); return render_template("bus_booking_form.html",clients=clients,booking=b)
...
def reports():
    conn=db()
    rows=conn.execute("""SELECT service,COUNT(*) count,COALESCE(SUM(total),0) total,
        COALESCE(SUM(paid),0) paid,COALESCE(SUM(expenses),0) expenses,
        COALESCE(SUM(total-paid-expenses),0) profit
        FROM transactions GROUP BY service ORDER BY count DESC""").fetchall()
    bus=conn.execute("""SELECT company,COUNT(*) count,COALESCE(SUM(total),0) total,
        COALESCE(SUM(paid),0) paid FROM bus_bookings GROUP BY company ORDER BY count DESC""").fetchall()
    conn.close(); return render_template("reports.html",rows=rows,bus=bus)
